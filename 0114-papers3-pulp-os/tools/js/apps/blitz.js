...
({
  id: 'blitz',
  title: 'Blitz Ink',
  subtitle: 'chess clock 5+3',
  version: 1,
  abi: 2,
  main: function (os, arg) {
    var z = os.state('blitz', function () {
      return { w: 300000, b: 300000, inc: 3000, run: 0, last: 0, moves: 0 };
    });
    var ui = {};
    function settle() {
      if (z.run !== 0) {
    ...
    ui.bt = text(function () { return os.fmtClock(z.b); }).size('xl').center();
    ui.bl = text(function () {
      return (z.run === 2 ? '> ' : '') + 'BLACK';
    }).size('xs').center();
    ui.wt = text(function () { return os.fmtClock(z.w); }).size('xl').center();
    ui.wl = text(function () {
    ...
    var zb = col().pad(20, os.M, 20, os.M).gap(6).add(ui.bt, ui.bl)
      .onTap(function () { hit(2); });
    var zw = col().pad(20, os.M, 20, os.M).gap(6).add(ui.wt, ui.wl)
      .onTap(function () { hit(1); });
    var body = col().pad(8, 0, 0, 0).gap(4)
      .add(zb, divider(6, 0), ui.mid, divider(6, 0), zw);
    p.header(os.chrome('BLITZ INK 5+3')).content(body)
      .footer(os.hintFooter('tap your side - hold = pause'));
