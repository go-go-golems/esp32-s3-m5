...
({
  id: 'ink',
  title: 'Ink',
  subtitle: 'the beauty of e-ink',
  version: 1,
  abi: 2,
  main: function (os, arg) {
    var st = { scene: 0, lastMin: -1 };
    ...
    var p = page('ink')
      ...
      .content(col().pad(6, os.M, 0, os.M).gap(8).add(cv, cap));
    ...
    function show(scene, full) {
      ...
      if (scene === 0) {
        st.lastMin = Math.floor(millis() / 60000);
        clockFace(st.lastMin);
        p.show(full ? 1 : 0);
      } else if (scene === 1) {
        field();
        p.show(1);
    ...
    p.on(G.TAP, function () { show((st.scene + 1) % 3, st.scene === 2); });
    p.on(G.TICK, function () {
      if (st.scene !== 0) { return; }
