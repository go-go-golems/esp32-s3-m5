/*
 * Tutorial 0016: AtomS3R backlight helpers.
 *
...
#if CONFIG_TUTORIAL_0016_BACKLIGHT_I2C_ENABLE
    uint8_t buf[2] = {reg, value};
    return i2c_master_write_to_device(
        BL_I2C_PORT,
        (uint8_t)CONFIG_TUTORIAL_0016_BL_I2C_ADDR,
        buf,
        sizeof(buf),
        pdMS_TO_TICKS(1000));
#else
...
static esp_err_t backlight_i2c_init(void) {
    ...
    ESP_LOGI(TAG, "backlight i2c init: port=%d scl=%d sda=%d addr=0x%02x reg=0x%02x",
             (int)BL_I2C_PORT,
             CONFIG_TUTORIAL_0016_BL_I2C_SCL_GPIO,
             CONFIG_TUTORIAL_0016_BL_I2C_SDA_GPIO,
             (unsigned)CONFIG_TUTORIAL_0016_BL_I2C_ADDR,
             (unsigned)CONFIG_TUTORIAL_0016_BL_I2C_REG);
    ...
    conf.scl_io_num = (gpio_num_t)CONFIG_TUTORIAL_0016_BL_I2C_SCL_GPIO;
    conf.sda_io_num = (gpio_num_t)CONFIG_TUTORIAL_0016_BL_I2C_SDA_GPIO;
    ...
    if (err == ESP_ERR_INVALID_STATE) {
        // Already installed by some other component.
        err = ESP_OK;
    }
    if (err != ESP_OK) {
...
static esp_err_t backlight_i2c_set(uint8_t brightness) {
    ...
    err = backlight_i2c_write_reg_u8((uint8_t)CONFIG_TUTORIAL_0016_BL_I2C_REG, brightness);
    if (err != ESP_OK) {
...
#if CONFIG_TUTORIAL_0016_BACKLIGHT_GATE_ENABLE
    gpio_reset_pin((gpio_num_t)CONFIG_TUTORIAL_0016_BACKLIGHT_GATE_GPIO);
    gpio_set_direction((gpio_num_t)CONFIG_TUTORIAL_0016_BACKLIGHT_GATE_GPIO, GPIO_MODE_OUTPUT);

#if CONFIG_TUTORIAL_0016_BACKLIGHT_GATE_ACTIVE_LOW
    const int level = on ? 0 : 1;
#else
    const int level = on ? 1 : 0;
#endif
    ESP_LOGI(TAG, "backlight gate gpio: pin=%d active_low=%d -> %s (level=%d)",
             CONFIG_TUTORIAL_0016_BACKLIGHT_GATE_GPIO,
             (int)(0
#if CONFIG_TUTORIAL_0016_BACKLIGHT_GATE_ACTIVE_LOW
                   + 1
#endif
             ...
             on ? "ON" : "OFF",
             level);
    gpio_set_level((gpio_num_t)CONFIG_TUTORIAL_0016_BACKLIGHT_GATE_GPIO, level);
#else
    (void)on;
#endif
}
...
#if CONFIG_TUTORIAL_0016_BACKLIGHT_I2C_ENABLE
    (void)backlight_i2c_set((uint8_t)CONFIG_TUTORIAL_0016_BL_BRIGHTNESS_ON);
#endif
}
