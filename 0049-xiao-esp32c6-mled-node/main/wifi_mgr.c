...
static const char *TAG = "mlednode_wifi_mgr";
...
static void wifi_event_handler(void *arg, esp_event_base_t base, int32_t id, void *data)
{
    ...
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_DISCONNECTED) {
        ...
        const bool do_retry = s_autoconnect && creds_present_unsafe() && (s_retry < CONFIG_MLEDNODE_WIFI_MAX_RETRY);
        unlock_mu();
        ...
        if (do_retry) {
            ...
            ESP_LOGE(TAG, "max retries reached (%d)", CONFIG_MLEDNODE_WIFI_MAX_RETRY);
        }
        return;
    }
...
esp_err_t wifi_mgr_start(void)
{
    ...
    lock_mu();
    s_autoconnect = CONFIG_MLEDNODE_WIFI_AUTOCONNECT_ON_BOOT && s_has_runtime;
    unlock_mu();
