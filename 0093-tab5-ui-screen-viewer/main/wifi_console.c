/*
 * Tab5 text echo demo: esp_console REPL for Wi-Fi persistence and debugging.
 */
...
static const char *TAG = "tab5_text_echo_console";

static const char *state_to_str(wifi_app_state_t state) {
    switch (state) {
...
static void print_ip4_line(const char *label, uint32_t ip4_host_order) {
    if (ip4_host_order == 0) {
        printf("%s=-\n", label);
        return;
    }
...
static int cmd_wifi(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "scan") == 0) {
        ...
        if (argc >= 3) {
            int parsed = 0;
            if (!parse_int(argv[2], &parsed) || parsed <= 0 || parsed > 200) {
        ...
        wifi_scan_entry_t *entries = calloc((size_t)max, sizeof(*entries));
        if (!entries) {
...
static void register_commands(void) {
    ...
    cmd.command = "wifi";
    cmd.help = "Wi-Fi STA config: wifi status|scan|set|save|connect|disconnect|clear";
    cmd.func = &cmd_wifi;
