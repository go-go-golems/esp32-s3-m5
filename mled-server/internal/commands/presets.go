...
var _ cmds.GlazeCommand = &PresetsCommand{}

func NewPresetsCommand() (*PresetsCommand, error) {
	...
	cmdDesc := cmds.NewCommandDescription(
		...
		cmds.WithLong(`List stored presets from a running mled-server instance.

Examples:
  mled-server presets
  mled-server presets --output json
`),
		cmds.WithFlags(flags...),
		cmds.WithLayersList(glazedLayer, commandSettingsLayer),
	)

	return &PresetsCommand{CommandDescription: cmdDesc}, nil
}

func (c *PresetsCommand) RunIntoGlazeProcessor(ctx context.Context, vals *values.Values, gp middlewares.Processor) error {
