/*
 * AtomS3R M12 QuickJS: native WiFi bring-up.
 *
 * Native esp_wifi (not esp_wifi_remote / ESP-Hosted). The AtomS3R M12 uses the ESP32-S3's on-chip WiFi.
 */
...
static esp_err_t nvs_open_wifi(nvs_handle_t *out) {
    ...
    return nvs_open(ATOMS3R_WIFI_NVS_NAMESPACE, NVS_READWRITE, out);
}
...
static esp_err_t save_current_credentials_to_nvs(void) {
    ...
    err = nvs_set_str(nvs, ATOMS3R_WIFI_NVS_KEY_SSID, s_runtime_ssid);
    if (err == ESP_OK) err = nvs_set_str(nvs, ATOMS3R_WIFI_NVS_KEY_PASS, s_runtime_pass);
...
static esp_err_t clear_saved_credentials_from_nvs(void) {
    ...
    esp_err_t err = nvs_open_wifi(&nvs);
    if (err != ESP_OK) return err;
    (void)nvs_erase_key(nvs, ATOMS3R_WIFI_NVS_KEY_SSID);
    (void)nvs_erase_key(nvs, ATOMS3R_WIFI_NVS_KEY_PASS);
    err = nvs_commit(nvs);
    nvs_close(nvs);
    return err;
}
...
static esp_err_t load_saved_credentials(void) {
    ...
    err = nvs_get_str(nvs, ATOMS3R_WIFI_NVS_KEY_SSID, ssid, &ssid_len);
    if (err != ESP_OK || ssid[0] == '\0') { nvs_close(nvs); return ESP_ERR_NOT_FOUND; }
    char pass[65] = {0};
    size_t pass_len = sizeof(pass);
    const esp_err_t pass_err = nvs_get_str(nvs, ATOMS3R_WIFI_NVS_KEY_PASS, pass, &pass_len);
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        ...
        const bool should_retry = s_autoconnect && s_has_runtime_creds && (s_retry_count < ATOMS3R_WIFI_MAX_RETRY);
        unlock_state();
...
esp_err_t wifi_app_start(void) {
    ...
    /* STA-only mode. USB Serial/JTAG remains the recovery path, so avoid SoftAP
     * address conflicts with guest networks that also use 192.168.4.0/22. */
    s_sta_netif = esp_netif_create_default_wifi_sta();
    (void)esp_netif_set_hostname(s_sta_netif, ATOMS3R_WIFI_HOSTNAME);
