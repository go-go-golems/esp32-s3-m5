...
static JSValue js_setTimeout(JSContext* ctx, JSValue* this_val, int argc, JSValue* argv) {
  (void)this_val;
  if (argc < 1) {
    return JS_ThrowTypeError(ctx, "setTimeout(fn, ms) requires at least 1 argument");
  }
  if (!JS_IsFunction(ctx, argv[0])) {
    return JS_ThrowTypeError(ctx, "setTimeout: fn must be a function");
  }

  int ms = 0;
  if (argc >= 2) {
    if (JS_ToInt32(ctx, &ms, argv[1])) {
      return JS_EXCEPTION;
    }
  }
  if (ms < 0) {
  ...
  if (JS_IsUndefined(ns) || JS_IsNull(ns)) {
    ns = JS_NewObject(ctx);
    (void)JS_SetPropertyStr(ctx, glob, "__lain", ns);
    ns = JS_GetPropertyStr(ctx, glob, "__lain");
  }
  ...
  const esp_err_t err = mqjs_0074_timers_schedule(id, (uint32_t)ms);
  if (err != ESP_OK) {
    (void)JS_SetPropertyUint32(ctx, cb, id, JS_NULL);
    return JS_ThrowInternalError(ctx, "setTimeout: schedule failed (%s)", esp_err_to_name(err));
  }
...
static JSValue js_clearTimeout(JSContext* ctx, JSValue* this_val, int argc, JSValue* argv) {
  ...
  (void)mqjs_0074_timers_cancel((uint32_t)id);
