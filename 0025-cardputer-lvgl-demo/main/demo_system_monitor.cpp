...
static void sample_cb(lv_timer_t *t) {
    ...
    const uint32_t lvgl_us_last = g_lvgl_handler_us_last;
    const uint32_t lvgl_us_avg = g_lvgl_handler_us_avg;
    const uint32_t fps_est = (lvgl_us_avg > 0) ? (1000000U / lvgl_us_avg) : 0U;

    char buf[128];
    snprintf(buf, sizeof(buf),
             "heap=%" PRIu32 "k dma=%" PRIu32 "k loop=%" PRIu32 "Hz lv=%" PRIu32 "/%" PRIu32 "us fps=%" PRIu32,
             heap_kb, dma_kb, st->loop_hz_ema.value, lvgl_us_last, lvgl_us_avg, fps_est);
    ...
    if (st->chart_fps && st->s_fps) {
        lv_chart_set_next_value(st->chart_fps, st->s_fps, (lv_coord_t)std::min<uint32_t>(fps_est, 60));
    }
}
...
lv_obj_t *demo_system_monitor_create(DemoManager *mgr) {
    ...
    st->chart_heap = make_chart(cont, 22);
    ...
    st->chart_dma = make_chart(cont, 22);
    ...
    st->chart_fps = make_chart(cont, 22);
