...
func newPrintCommand() (*PrintCommand, error) {
	...
	desc := cmds.NewCommandDescription(
		...
		cmds.WithLong(`Render a JSON or YAML layout, convert it to a 1-bit bitmap, and POST it to stoms3r.
...
`),
		cmds.WithFlags(
			fields.New("layout", fields.TypeObjectFromFile, fields.WithHelp("Layout object file to print. Accepts JSON or YAML.")),
...
func (c *PrintCommand) RunIntoGlazeProcessor(ctx context.Context, vals *values.Values, gp middlewares.Processor) error {
	...
	opts := RenderOptions{
		Selector:       stringFromRenderOptions(fileRenderOptions, "selector", s.Selector),
		Threshold:      uint8(intFromRenderOptions(fileRenderOptions, "threshold", s.Threshold)),
		ViewportWidth:  intFromRenderOptions(fileRenderOptions, "viewportWidth", s.ViewportWidth),
		ViewportHeight: intFromRenderOptions(fileRenderOptions, "viewportHeight", s.ViewportHeight),
