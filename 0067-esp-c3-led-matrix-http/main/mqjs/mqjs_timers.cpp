...
static const char* TAG = "0067_js_timers";

namespace {

constexpr uint32_t kMaxTimers = CONFIG_TUTORIAL_0067_JS_MAX_TIMERS;
...
static JSValue get_timer_cb(JSContext* ctx, uint32_t id, JSValue* out_cb_table) {
  ...
  if (JS_IsUndefined(cb) || JS_IsNull(cb)) return JS_UNDEFINED;

  JSValue fn = JS_GetPropertyUint32(ctx, cb, id);
  if (out_cb_table) *out_cb_table = cb;
  return fn;
}
...
static esp_err_t job_fire_timeout(JSContext* ctx, void* user) {
  ...
  if (threw) log_js_exception(ctx, "timeout callback threw");
...
static void handle_cmd(const Cmd& c) {
  ...
  if (c.type == CMD_CANCEL) {
    ...
    if (idx >= 0) s_timers[idx].active = false;
    return;
  }
...
static void timers_task(void* arg) {
  (void)arg;
  ESP_LOGI(TAG, "task start (max_timers=%u)", (unsigned)kMaxTimers);
...
esp_err_t mqjs_0067_timers_start(mqjs_service_t* svc) {
  ...
  BaseType_t ok = xTaskCreate(&timers_task, "0067_js_tmr", 4096, nullptr, 8, &s_task);
  if (ok != pdPASS) {
    vQueueDelete(s_q);
    s_q = nullptr;
    s_task = nullptr;
...
void mqjs_0067_timers_stop(void) {
  if (s_task) {
    vTaskDelete(s_task);
    s_task = nullptr;
  }
...
esp_err_t mqjs_0067_timers_schedule(uint32_t id, uint32_t delay_ms) {
...
esp_err_t mqjs_0067_timers_cancel(uint32_t id) {
  if (!s_q) return ESP_ERR_INVALID_STATE;
  if (id == 0) return ESP_ERR_INVALID_ARG;
  Cmd c = {};
  c.type = CMD_CANCEL;
...
esp_err_t mqjs_0067_timers_cancel_all(void) {
  if (!s_q) return ESP_ERR_INVALID_STATE;
  Cmd c = {};
  c.type = CMD_CANCEL_ALL;
  if (xQueueSend(s_q, &c, pdMS_TO_TICKS(50)) != pdTRUE) return ESP_ERR_TIMEOUT;
  return ESP_OK;
}
