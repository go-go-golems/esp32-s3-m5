...
static void usage(void)
{
    ...
    printf("  matrix scroll on <TEXT> [fps] [pause_ms]\n");
    printf("  matrix scroll wave <TEXT> [fps] [pause_ms]\n");
    printf("  matrix scroll off\n");
    printf("  matrix anim drop <TEXT> [fps] [pause_ms]\n");
...
static int cmd_matrix(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "status") == 0) {
        ...
        printf("ok: ready=%s mode=%s chain=%d width=%d spi_hz=%d fps=%u pause_ms=%u reverse=%s flipv=%s text=%s\n",
               st.ready ? "yes" : "no",
               mode,
               st.chain_len,
               st.width,
    ...
    if (strcmp(argv[1], "scroll") == 0) {
        ...
        return matrix_engine_start_scroll(argv[3], fps, pause_ms, wave) == ESP_OK ? 0 : 1;
    }
    ...
    if (strcmp(argv[1], "anim") == 0) {
        ...
        if (argc >= 5) fps = (uint32_t)strtoul(argv[4], NULL, 0);
        if (argc >= 6) pause_ms = (uint32_t)strtoul(argv[5], NULL, 0);
        return matrix_engine_start_drop(argv[3], fps, pause_ms) == ESP_OK ? 0 : 1;
    }
