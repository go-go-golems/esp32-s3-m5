/*
 * Tab5 text echo demo: Wi-Fi bring-up, persistence, and network state.
 */
...
static const char *TAG = "tab5_text_echo_wifi";

#define TAB5_WIFI_AP_SSID "Tab5-Text-Echo"
#define TAB5_WIFI_AP_PASSWORD "tab5echo"
#define TAB5_WIFI_AP_CHANNEL 1
#define TAB5_WIFI_AP_MAX_CONN 4
#define TAB5_WIFI_HOSTNAME "tab5-text-echo"
#define TAB5_WIFI_NVS_NAMESPACE "wifi"
#define TAB5_WIFI_NVS_KEY_SSID "ssid"
#define TAB5_WIFI_NVS_KEY_PASS "pass"
#define TAB5_WIFI_MAX_RETRY 10
...
static esp_err_t nvs_open_wifi(nvs_handle_t *out) {
    ...
    return nvs_open(TAB5_WIFI_NVS_NAMESPACE, NVS_READWRITE, out);
}

static esp_err_t save_current_credentials_to_nvs(void) {
    if (!s_has_runtime_creds || s_runtime_ssid[0] == '\0') {
    ...
    err = nvs_set_str(nvs, TAB5_WIFI_NVS_KEY_SSID, s_runtime_ssid);
    if (err == ESP_OK) {
        err = nvs_set_str(nvs, TAB5_WIFI_NVS_KEY_PASS, s_runtime_pass);
    }
    if (err == ESP_OK) {
        err = nvs_commit(nvs);
    }
...
static esp_err_t clear_saved_credentials_from_nvs(void) {
    ...
    (void)nvs_erase_key(nvs, TAB5_WIFI_NVS_KEY_SSID);
    (void)nvs_erase_key(nvs, TAB5_WIFI_NVS_KEY_PASS);
    err = nvs_commit(nvs);
    nvs_close(nvs);
    return err;
}
...
static esp_err_t load_saved_credentials(void) {
    ...
    err = nvs_get_str(nvs, TAB5_WIFI_NVS_KEY_SSID, ssid, &ssid_len);
    ...
    const esp_err_t pass_err = nvs_get_str(nvs, TAB5_WIFI_NVS_KEY_PASS, pass, &pass_len);
    if (pass_err != ESP_OK) {
        pass[0] = '\0';
    }
    nvs_close(nvs);
...
static void set_ap_config(wifi_config_t *out_cfg) {
    wifi_config_t ap_cfg = {0};

    strlcpy((char *)ap_cfg.ap.ssid, TAB5_WIFI_AP_SSID, sizeof(ap_cfg.ap.ssid));
    ap_cfg.ap.ssid_len = (uint8_t)strlen((const char *)ap_cfg.ap.ssid);

    strlcpy((char *)ap_cfg.ap.password, TAB5_WIFI_AP_PASSWORD, sizeof(ap_cfg.ap.password));
    ap_cfg.ap.channel = TAB5_WIFI_AP_CHANNEL;
    ap_cfg.ap.max_connection = TAB5_WIFI_AP_MAX_CONN;
...
static void set_sta_config(wifi_config_t *out_cfg) {
...
static void wifi_event_handler(void *arg,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_START) {
        lock_state();
        update_ap_ip_locked();
        const uint32_t ip4 = s_status.ap_ip4;
        unlock_state();
        log_host_ip("SoftAP", ip4);
        log_host_urls();
        return;
    }
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        if (e) {
            ESP_LOGI(TAG, "AP client connected: %02x:%02x:%02x:%02x:%02x:%02x aid=%d",
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STADISCONNECTED) {
        ...
        if (e) {
            ESP_LOGI(TAG, "AP client disconnected: %02x:%02x:%02x:%02x:%02x:%02x aid=%d",
                     e->mac[0], e->mac[1], e->mac[2], e->mac[3], e->mac[4], e->mac[5], e->aid);
        }
        return;
    }
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (do_connect) {
            ESP_LOGI(TAG, "STA start: connecting to saved credentials...");
            esp_err_t err = esp_wifi_connect();
            if (err == ESP_OK) {
                lock_state();
                s_status.state = WIFI_APP_STATE_CONNECTING;
                unlock_state();
            } else {
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        ...
        if (s_status.state != WIFI_APP_STATE_UNINIT) {
            s_status.state = WIFI_APP_STATE_IDLE;
        }
        const bool should_retry = s_autoconnect && s_has_runtime_creds && (s_retry_count < TAB5_WIFI_MAX_RETRY);
        unlock_state();
        ...
        if (should_retry) {
            s_retry_count += 1;
            esp_wifi_connect();
        } else if (s_autoconnect && s_has_runtime_creds) {
            ESP_LOGE(TAG, "STA retry budget exhausted (%d)", TAB5_WIFI_MAX_RETRY);
        }
        return;
    }
    ...
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        const uint32_t ip4 = s_status.sta_ip4;
        unlock_state();
...
static esp_err_t configure_apsta_mode(void) {
    if (!s_ap_netif) {
    ...
    if (!s_sta_netif) {
        ...
        (void)esp_netif_set_hostname(s_sta_netif, TAB5_WIFI_HOSTNAME);
    }

    wifi_config_t ap_cfg = {0};
    set_ap_config(&ap_cfg);
...
esp_err_t wifi_app_start(void) {
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGW(TAG, "Failed to load Wi-Fi credentials from NVS: %s", esp_err_to_name(err));
    }
    ...
    update_ap_ip_locked();
    update_sta_ip_locked();
    const uint32_t ap_ip4 = s_status.ap_ip4;
    const uint32_t sta_ip4 = s_status.sta_ip4;
    unlock_state();

    if (ap_ip4 == 0) {
        ESP_LOGI(TAG, "SoftAP browse: http://192.168.4.1/");
    } else {
        log_host_ip("SoftAP", ap_ip4);
    }
    if (sta_ip4 != 0) {
        log_host_ip("STA", sta_ip4);
    }
...
esp_err_t wifi_app_clear_credentials(void) {
    ...
    s_runtime_ssid[0] = '\0';
    s_runtime_pass[0] = '\0';
    s_has_runtime_creds = false;
    s_has_saved_creds = false;
    s_autoconnect = false;
    s_retry_count = 0;
    s_status.sta_ip4 = 0;
    s_status.last_disconnect_reason = -1;
