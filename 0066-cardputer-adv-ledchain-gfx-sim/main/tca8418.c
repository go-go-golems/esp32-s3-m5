...
static esp_err_t dev_write_reg8(i2c_port_t port, uint8_t addr7, uint8_t reg, uint8_t val)
{
    uint8_t buf[2] = {reg, val};
    return i2c_master_write_to_device(port, addr7, buf, sizeof(buf), pdMS_TO_TICKS(50));
}

static esp_err_t dev_read_reg8(i2c_port_t port, uint8_t addr7, uint8_t reg, uint8_t *out)
{
    if (!out) return ESP_ERR_INVALID_ARG;
    return i2c_master_write_read_device(port, addr7, &reg, 1, out, 1, pdMS_TO_TICKS(50));
}
...
{
    if (!dev) return ESP_ERR_INVALID_ARG;
    return dev_write_reg8(dev->port, dev->addr7, reg, val);
}

esp_err_t tca8418_read_reg8(const tca8418_t *dev, uint8_t reg, uint8_t *out)
{
    if (!dev) return ESP_ERR_INVALID_ARG;
    return dev_read_reg8(dev->port, dev->addr7, reg, out);
}
