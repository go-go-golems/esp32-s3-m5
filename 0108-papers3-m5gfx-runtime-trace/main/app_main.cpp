...
constexpr char kTraceVariant[] = "timing";
#else
constexpr char kTraceVariant[] = "off";
#endif
...
bool DrawTransaction(epd_mode_t mode, bool full, DrawFn&& draw)
{
    ...
    const uint32_t trace_operation = papers3::trace::BeginOperation(static_cast<uint32_t>(mode), full);
    ...
    papers3::trace::EndOperation(trace_operation, true, elapsed);
    return true;
}
...
bool ExplicitDisplayRange(int32_t x, int32_t y, int32_t w, int32_t h)
{
    ...
    const int64_t started = esp_timer_get_time();
    const uint32_t trace_operation = papers3::trace::BeginOperation(UINT32_MAX, false);
    M5.Display.waitDisplay();
    M5.Display.display(x, y, w, h);
    M5.Display.waitDisplay();
    const uint64_t elapsed = static_cast<uint64_t>(esp_timer_get_time() - started);
    RecordUpdate(false, elapsed);
    papers3::trace::EndOperation(trace_operation, true, elapsed);
    return true;
}
...
void PrintHelp()
{
    ...
    std::printf("  epd trace status|dump|reset\n");
...
int CommandEpd(int argc, char** argv)
{
    ...
    if (std::strcmp(argv[1], "status") == 0) {
        DisplayGuard guard;
        PrintStatus();
        papers3::trace::PrintStatus();
        return 0;
    }

    if (std::strcmp(argv[1], "trace") == 0) {
        if (argc != 3) {
            std::printf("usage: epd trace status|dump|reset\n");
            return 1;
        }
        DisplayGuard guard;
        if (!guard.locked()) {
        ...
        if (std::strcmp(argv[2], "status") == 0) papers3::trace::PrintStatus();
        else if (std::strcmp(argv[2], "dump") == 0) papers3::trace::DumpJsonLines();
        else if (std::strcmp(argv[2], "reset") == 0) papers3::trace::Reset();
        else {
            std::printf("usage: epd trace status|dump|reset\n");
            return 1;
        }
...
extern "C" void app_main(void)
{
    ...
    std::printf("cell=%s idf=%s m5gfx=%s m5unified=%s trace=%s\n", PAPERS3_MATRIX_CELL,
                esp_get_idf_version(), PAPERS3_M5GFX_SHA, PAPERS3_M5UNIFIED_SHA, kTraceVariant);
    ...
    if (g_display_ready) {
        M5.Display.setRotation(1);
        ConfigureTextDefaults();
        // No display transaction is issued at boot. Runtime traces and physical
        // drive begin only after an explicit console command in a later gate.
    } else {
