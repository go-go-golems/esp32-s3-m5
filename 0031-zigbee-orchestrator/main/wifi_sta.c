/*
 * WiFi STA bring-up + runtime configuration (console-friendly).
 */
...
static void status_log_ip(uint32_t ip4_host_order) {
    ...
    ESP_LOGI(TAG, "browse: http://" IPSTR "/ (or /v1/health)", IP2STR(&ip));
}
...
static void wifi_event_handler(void *arg, esp_event_base_t base, int32_t id, void *data) {
    ...
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_START) {
        lock_mu();
        s_started = true;
        if (s_status.state == WIFI_STA_STATE_UNINIT) {
    ...
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_DISCONNECTED) {
        ...
        const bool do_retry = s_autoconnect && creds_present_unsafe() && (s_retry < CONFIG_TUTORIAL_0029_WIFI_MAX_RETRY);
        unlock_mu();
        ...
        if (do_retry) {
            ...
            ESP_LOGE(TAG, "max retries reached (%d)", CONFIG_TUTORIAL_0029_WIFI_MAX_RETRY);
        }
        return;
    }
...
esp_err_t hub_wifi_start(void) {
    ...
    const bool kconfig_has_creds = (CONFIG_TUTORIAL_0029_WIFI_SSID[0] != '\0');
    ...
    if (!s_has_runtime && kconfig_has_creds) {
        ESP_LOGI(TAG, "using WiFi credentials from Kconfig (ssid=%s)", CONFIG_TUTORIAL_0029_WIFI_SSID);
        (void)hub_wifi_set_credentials(CONFIG_TUTORIAL_0029_WIFI_SSID, CONFIG_TUTORIAL_0029_WIFI_PASSWORD, false);
    }
...
esp_err_t hub_wifi_disconnect(void) {
    ...
    s_autoconnect = false;
    s_status.ip4 = 0;
    if (s_status.state != WIFI_STA_STATE_UNINIT) s_status.state = WIFI_STA_STATE_IDLE;
    unlock_mu();
