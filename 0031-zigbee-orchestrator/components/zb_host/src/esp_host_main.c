...
static void esp_host_main_task(void *pv) 
{
    ...
    while (dev->run) {
        ...
        const esp_err_t err = esp_host_process_event(dev, &host_ctx);
        if (err != ESP_OK) {
            // The UART can carry boot ROM logs and other noise; treat framing errors
            // as a dropped input chunk rather than a fatal condition.
            if (host_ctx.event == HOST_EVENT_INPUT) {
                ESP_LOGW(TAG, "Process input event failed: %s", esp_err_to_name(err));
                continue;
            }
            ESP_LOGE(TAG, "Process event fail: %s", esp_err_to_name(err));
            break;
        }
    }
...
esp_err_t esp_host_start(void)
{
    ...
    s_host_dev.queue = xQueueCreate(HOST_EVENT_QUEUE_LEN, sizeof(esp_host_ctx_t));
    if (!s_host_dev.queue) {
        return ESP_ERR_NO_MEM;
    }
    ...
    esp_err_t ret = esp_host_bus_start(s_host_dev.bus);
    if (ret != ESP_OK) {
    ...
    if (xTaskCreate(esp_host_main_task, "esp_host_main_task", HOST_TASK_STACK, &s_host_dev, HOST_TASK_PRIORITY, NULL) != pdTRUE) {
