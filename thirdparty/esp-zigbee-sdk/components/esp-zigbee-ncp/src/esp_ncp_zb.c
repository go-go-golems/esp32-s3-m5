...
static bool s_zb_stack_inited = false;
static uint32_t s_channel_mask = 0;
static bool s_nvram_erase_at_start = false;
static bool s_link_key_exchange_required = true;
static uint8_t s_tc_preconfigure_key[16] = {
...
{
    if (!s_zb_stack_inited) {
        return;
    }

    if (s_channel_mask) {
        esp_err_t err = esp_zb_set_channel_mask(s_channel_mask);
        ESP_LOGI(TAG, "apply channel mask: 0x%08" PRIx32 " (%s)", s_channel_mask, esp_err_to_name(err));
    }
    if (s_primary_channel) {
        esp_err_t err = esp_zb_set_primary_network_channel_set(s_primary_channel);
        ESP_LOGI(TAG, "apply primary channel mask: 0x%08" PRIx32 " (%s)", s_primary_channel, esp_err_to_name(err));
    }
}
...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    switch (sig_type) {
        ...
        case ESP_ZB_ZDO_SIGNAL_DEVICE_ANNCE:
            ...
            if (dev_annce_params) {
                ESP_LOGI(TAG,
                         "New device commissioned or rejoined (short: 0x%04hx, ieee: %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x, cap: 0x%02x)",
                         dev_annce_params->device_short_addr,
                         dev_annce_params->ieee_addr[7],
                         dev_annce_params->ieee_addr[6],
                         dev_annce_params->ieee_addr[5],
                         dev_annce_params->ieee_addr[4],
                         dev_annce_params->ieee_addr[3],
                         dev_annce_params->ieee_addr[2],
                         dev_annce_params->ieee_addr[1],
                         dev_annce_params->ieee_addr[0],
                         dev_annce_params->capability);
            } else {
                ESP_LOGW(TAG, "New device commissioned or rejoined (missing params)");
            }
        ...
        case ESP_ZB_ZDO_SIGNAL_LEAVE:
            ...
            if (dev_leave_params) {
                ESP_LOGI(TAG,
                         "Leave Indication (short: 0x%04hx, rejoin=%u, ieee: %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x)",
                         dev_leave_params->short_addr,
                         (unsigned)dev_leave_params->rejoin,
                         dev_leave_params->device_addr[7],
                         dev_leave_params->device_addr[6],
                         dev_leave_params->device_addr[5],
                         dev_leave_params->device_addr[4],
                         dev_leave_params->device_addr[3],
                         dev_leave_params->device_addr[2],
                         dev_leave_params->device_addr[1],
                         dev_leave_params->device_addr[0]);
            } else {
                ESP_LOGW(TAG, "Leave Indication (missing params)");
            }
        ...
        case ESP_ZB_ZDO_SIGNAL_DEVICE_UPDATE: {
            ...
            if (!u) break;
            ESP_LOGI(TAG,
                     "Device update (status=0x%02x short=0x%04hx ieee=%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x)",
                     (unsigned)u->status,
                     u->short_addr,
                     u->long_addr[7],
                     u->long_addr[6],
        ...
        case ESP_ZB_ZDO_SIGNAL_DEVICE_AUTHORIZED: {
            esp_zb_zdo_signal_device_authorized_params_t *a =
                (esp_zb_zdo_signal_device_authorized_params_t *)esp_zb_app_signal_get_params(p_sg_p);
            if (!a) break;
            ESP_LOGI(TAG,
                     "Device authorized (type=0x%02x status=0x%02x short=0x%04hx ieee=%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x)",
...
static esp_err_t esp_ncp_zb_network_primary_channel_set_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    if (ret == ESP_OK) {
        uint32_t mask = 0;
        memcpy(&mask, input, sizeof(mask));
        s_primary_channel = mask;

        // The host may set this before `esp_zb_init()` is called; applying it
        // too early can crash in some Zigbee SDK versions. Store it and apply
        // after initialization (in form-network) instead.
        if (s_zb_stack_inited) {
            ret = esp_zb_set_primary_network_channel_set(mask);
...
static esp_err_t esp_ncp_zb_channel_set_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    if (ret == ESP_OK) {
        ...
        if (s_zb_stack_inited) {
            ret = esp_zb_set_channel_mask(mask);
            status = (ret == ESP_OK) ? ESP_NCP_SUCCESS : ESP_NCP_ERR_FATAL;
        }
    }
...
static esp_err_t esp_ncp_zb_form_network_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    if (input) {
        // Ensure NVRAM erase policy is set before the stack starts (it controls
        // whether zb_storage is cleared before parameters are loaded).
        esp_zb_nvram_erase_at_start(s_nvram_erase_at_start);

        // `input` points into a request frame buffer owned by the NCP transport;
        // ensure the Zigbee stack never keeps a pointer to ephemeral memory.
        static esp_zb_cfg_t s_zb_cfg;
        memcpy(&s_zb_cfg, input, sizeof(s_zb_cfg));
        esp_zb_init(&s_zb_cfg);
        s_zb_stack_inited = true;

        // Security defaults and overrides (TCLK / key exchange requirement).
        esp_zb_secur_TC_standard_preconfigure_key_set(s_tc_preconfigure_key);
        esp_zb_secur_link_key_exchange_required_set(s_link_key_exchange_required);

        // Apply channel masks after init (order: general channel mask, then primary set).
        if (s_channel_mask) {
            (void)esp_zb_set_channel_mask(s_channel_mask);
        }
        // Apply a previously configured primary channel mask after init.
        if (s_primary_channel) {
            (void)esp_zb_set_primary_network_channel_set(s_primary_channel);
        }
    }
...
static esp_err_t esp_ncp_zb_permit_joining_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    esp_ncp_status_t status = (ret == ESP_OK) ? ESP_NCP_SUCCESS : ESP_NCP_BAD_ARGUMENT;

    if (ret == ESP_OK) {
        const uint8_t seconds = *(const uint8_t *)input;
        ESP_LOGI(TAG, "permit_join seconds=%u", (unsigned)seconds);
...
static esp_err_t esp_ncp_zb_nwk_link_key_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    *output = calloc(1, *outlen);
    ...
    if (*output) {
        ...
        memcpy((*output) + sizeof(esp_zb_ieee_addr_t), s_tc_preconfigure_key, sizeof(s_tc_preconfigure_key));
    }
...
static esp_err_t esp_ncp_zb_nwk_link_key_set_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    if (ret == ESP_OK) {
        memcpy(s_tc_preconfigure_key, input, sizeof(s_tc_preconfigure_key));
        if (s_zb_stack_inited) {
            esp_zb_secur_TC_standard_preconfigure_key_set(s_tc_preconfigure_key);
        }
    }
...
static esp_err_t esp_ncp_zb_nwk_security_mode_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    *output = calloc(1, *outlen);
    ...
    if (*output) {
        *(*output) = s_link_key_exchange_required ? ESP_NCP_PRECONFIGURED_NETWORK_KEY : ESP_NCP_NO_SECURITY;
    }
...
static esp_err_t esp_ncp_zb_nwk_security_mode_set_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    ...
    if (ret == ESP_OK) {
        const uint8_t mode = *(const uint8_t *)input;
...
static esp_err_t esp_ncp_zb_nvram_erase_at_start_get_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    *outlen = sizeof(uint8_t);
    *output = calloc(1, *outlen);

    if (*output) {
        *(*output) = s_nvram_erase_at_start ? 1 : 0;
    }

    return (*output) ? ESP_OK : ESP_ERR_NO_MEM;
}

static esp_err_t esp_ncp_zb_nvram_erase_at_start_set_fn(const uint8_t *input, uint16_t inlen, uint8_t **output, uint16_t *outlen)
{
    esp_err_t ret = (input && inlen == sizeof(uint8_t)) ? ESP_OK : ESP_ERR_INVALID_ARG;
    esp_ncp_status_t status = (ret == ESP_OK) ? ESP_NCP_SUCCESS : ESP_NCP_ERR_FATAL;

    if (ret == ESP_OK) {
        s_nvram_erase_at_start = (*(const uint8_t *)input) ? true : false;
        if (s_zb_stack_inited) {
            esp_zb_nvram_erase_at_start(s_nvram_erase_at_start);
        }
    }
...
static const esp_ncp_zb_func_t ncp_zb_func_table[] = {
    ...
    {ESP_NCP_NETWORK_NVRAM_ERASE_AT_START_SET, esp_ncp_zb_nvram_erase_at_start_set_fn},
    {ESP_NCP_NETWORK_NVRAM_ERASE_AT_START_GET, esp_ncp_zb_nvram_erase_at_start_get_fn},
