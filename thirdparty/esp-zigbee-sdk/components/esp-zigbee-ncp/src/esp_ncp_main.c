...
esp_err_t esp_ncp_start(void)
{
    ...
    if (ret != ESP_OK) {
        vQueueDelete(s_ncp_dev.queue);
        s_ncp_dev.queue = NULL;
        s_ncp_dev.run = false;
        return ret;
    }

    if (xTaskCreate(esp_ncp_main_task, "esp_ncp_main_task", NCP_TASK_STACK, &s_ncp_dev, NCP_TASK_PRIORITY, NULL) != pdTRUE) {
        esp_ncp_bus_stop(s_ncp_dev.bus);
        vQueueDelete(s_ncp_dev.queue);
        s_ncp_dev.queue = NULL;
        s_ncp_dev.run = false;
        return ESP_FAIL;
