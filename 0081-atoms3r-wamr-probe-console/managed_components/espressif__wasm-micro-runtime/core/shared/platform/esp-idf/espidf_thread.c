...
os_self_thread(void)
{
    /* This project currently drives WAMR from esp_console, which runs on a
     * normal FreeRTOS task rather than a pthread-created task. pthread_self()
     * asserts in that context on ESP-IDF. For the current interpreter-only
     * single-threaded execution path, a stable task handle is sufficient as
     * the native-thread identity used by WAMR. */
