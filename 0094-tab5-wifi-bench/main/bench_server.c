...
static esp_err_t bench_upload_post(httpd_req_t *req) {
    ...
    /* Cap at 4 MB to avoid exhausting SPIRAM. */
    const size_t max_recv = 4 * 1024 * 1024;
    ...
    uint8_t *recv_buf = heap_caps_malloc((size_t)content_len, MALLOC_CAP_SPIRAM);
    if (!recv_buf) {
    ...
    recv_segment_t *segs = heap_caps_malloc(MAX_SEGMENTS * sizeof(recv_segment_t), MALLOC_CAP_SPIRAM);
    if (!segs) {
        free(recv_buf);
    ...
    if (deflated) {
        /* Use the ?size= query parameter as the expected decompressed size.
         * Without this hint, we cannot allocate a large enough output buffer
         * because the compressed size bears no relation to the decompressed size
         * (e.g. 7 KB compressed -> 1.8 MB decompressed for incrementing patterns). */
        const int expected_size = query_int(req, "size", 0);
        const size_t max_dec = 4 * 1024 * 1024;
        const size_t dec_buf_size = (expected_size > 0 && (size_t)expected_size <= max_dec)
            ? (size_t)expected_size
            : (((size_t)received * 2 > max_dec) ? max_dec : (size_t)received * 2);
        uint8_t *dec_buf = heap_caps_malloc(dec_buf_size, MALLOC_CAP_SPIRAM);
        if (!dec_buf) {
            free(recv_buf);
            free(segs);
            send_json_error(req, 500, "decompress alloc failed");
            return ESP_OK;
        }

        T3 = esp_timer_get_time();
        dec_len = tinfl_decompress_mem_to_mem(
        ...
        if (dec_len == TINFL_DECOMPRESS_MEM_TO_MEM_FAILED) {
            free(recv_buf);
            free(segs);
...
static esp_err_t bench_download_get(httpd_req_t *req) {
    const int size = query_int(req, "size", 1843200);  /* 1.8 MB default */
    const bool deflated = (query_int(req, "compress", 0) == 1);

    if (size <= 0 || size > 4 * 1024 * 1024) {
        send_json_error(req, 400, "size must be 1..4194304");
        return ESP_OK;
    }
    ...
    /* Incrementing byte pattern — compressible but not trivial. */
    ...
    if (deflated) {
        ...
        /* For download deflate, we send the raw pattern and let the
         * client/browser handle decompression. But we don't have a
         * compressor on the ESP32 (miniz in ROM is decompress-only).
         * So we just send raw and note that deflate is unsupported. */
...
static esp_err_t bench_ping_post(httpd_req_t *req) {
    ...
    uint8_t *buf = heap_caps_malloc((size_t)content_len, MALLOC_CAP_SPIRAM);
    if (!buf) {
