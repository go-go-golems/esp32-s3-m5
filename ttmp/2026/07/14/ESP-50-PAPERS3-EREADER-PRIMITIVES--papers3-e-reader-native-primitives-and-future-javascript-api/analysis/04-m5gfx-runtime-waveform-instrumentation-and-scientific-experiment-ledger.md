...
RelatedFiles:
    ...
    - Path: repo://ttmp/2026/07/14/ESP-50-PAPERS3-EREADER-PRIMITIVES--papers3-e-reader-native-primitives-and-future-javascript-api/scripts/41-stage-f2-no-reset.sh
      Note: No-reset F2 stage
    - Path: repo://ttmp/2026/07/14/ESP-50-PAPERS3-EREADER-PRIMITIVES--papers3-e-reader-native-primitives-and-future-javascript-api/scripts/42-arm-f2-manual-reset-capture.sh
    ...
    - Path: repo://ttmp/2026/07/14/ESP-50-PAPERS3-EREADER-PRIMITIVES--papers3-e-reader-native-primitives-and-future-javascript-api/scripts/experiments/EXP-20260715-009-factory-f1-dynamic-density/01-preregistration.md
      Note: Fixed-head/camera F1 source-proxy comparison gate
    ...
    - Path: repo://ttmp/2026/07/14/ESP-50-PAPERS3-EREADER-PRIMITIVES--papers3-e-reader-native-primitives-and-future-javascript-api/scripts/experiments/EXP-20260715-011-factory-f2-ring-density/04-failed-attempt.md
...
A first passive physical validation used the Printalyzer's normal calibrated reflection workflow, not remote diagnostics. After six operator-identified invalid setup readings, three CAL-LO results were exactly `R+0.05D` and three CAL-HI results were exactly `R+1.49D`; the latter matched the reported reference value. The collector sent no serial input. This passes event delivery and repeatability only at BASIC mode's 0.01 D display precision. A future `SM FORMAT,EXT` normal-measurement run can expose full density and raw/corrected counts without enabling raw streaming, but panel geometry and button-to-result latency remain unqualified.

The raw collector now snapshots `GC LIGHT`, `GC GAIN`, `GC SLOPE`, and `GC REFL` before entering remote mode and reproduces the installed v1.1.0 density formula per `GD S` sample. It normalizes channel 0 by integration time and calibrated gain, applies the log-space slope polynomial, and maps corrected counts through the reflection CAL-LO/CAL-HI line. A result is rejected on saturation, invalid input, or LED-duty mismatch. Even a valid value remains a **single-sample host estimate**, not an assertion that diagnostic streaming is identical to the normal auto-gain, 200 ms, two-reading average path.

The first physical raw run sampled a stationary operator-identified white region approximately middle-ish on the PaperS3 while the board remained in ROM download mode. Forty-six post-settling samples at gain 2, 100 ms, and duty 128 yielded channel-0 mean 1929.3 counts and density estimate `0.678142 ± 0.000625 D`, with range `0.002243 D`, zero saturation, zero invalid estimates, and acknowledged STOP/light-off/remote-exit cleanup. This strongly passes point-signal stability and headroom, but the absolute value remains unqualified until lift/reposition sensitivity and raw CAL-LO/CAL-HI agreement are measured. The operator confirmed the head did not move and the LED was off afterward; no obvious panel change was reported, though that visual confirmation was uncertain.

Lift/reseat experiment 005 changed the mean to `0.600075 D`, a `0.078067 D` delta that failed the preregistered `0.03 D` placement threshold despite excellent within-run stability. Experiments 006 and 007 then captured transient excursions of roughly the same magnitude. The operator later noted that the sampled app background also changes, making the hand-wave causal test inconclusive rather than proof of ambient leakage. The defensible conclusion is narrower: fixed-point density is sensitive enough to detect these changes, while absolute cross-run comparison after manual repositioning is not qualified. Experiment 008 therefore preregisters an exact-F0 dynamic trace with the head fixed for the entire sequence; it does not waive the need for video-based spatial scoring or authorize F1 automatically.
