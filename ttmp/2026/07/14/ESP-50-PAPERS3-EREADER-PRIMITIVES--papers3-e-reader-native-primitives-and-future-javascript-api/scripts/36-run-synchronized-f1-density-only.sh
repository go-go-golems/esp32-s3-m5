...
EXP="$TICKET/scripts/experiments/EXP-20260715-010-factory-f1-density-only-control"
RAW="$EXP/raw-dynamic-f1.jsonl"
...
while (($#)); do
  case "$1" in
    ...
    *) echo "usage: $0 [--check|--execute --confirm RUN-DENS-F1-NOVIDEO]" >&2; exit 2 ;;
  esac
done

[[ -d "$EXP" ]] || { echo "missing experiment: $EXP" >&2; exit 3; }
(cd "$EXP" && sha256sum -c preregistration.sha256 >/dev/null)
"$TICKET/scripts/27-run-factory-comparison-flash.sh" --check --treatment f1 >/dev/null
...
[[ "$CONFIRM" == RUN-DENS-F1-NOVIDEO ]] || { echo 'execute requires --confirm RUN-DENS-F1-NOVIDEO' >&2; exit 4; }
...
"$TICKET/scripts/27-run-factory-comparison-flash.sh" \
  --execute --treatment f1 --confirm RUN-F1 >"$FLASH_RUNNER_LOG" 2>&1
