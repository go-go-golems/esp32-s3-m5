...
EXP="$TICKET/scripts/experiments/EXP-20260715-014-factory-f2-readonly-reconnect-ring"
RAW="$EXP/read-only-reset-capture.jsonl"
CONSOLE="$EXP/capture-console.log"
SESSION='esp50-f2-readonly-reset'
MODE=check
CONFIRM=''
while (($#)); do
  case "$1" in
    ...
    *) echo "usage: $0 [--check|--execute --confirm ARM-F2-READONLY-RESET]" >&2; exit 2 ;;
  esac
done
...
[[ "$CONFIRM" == ARM-F2-READONLY-RESET ]] || { echo 'execute requires --confirm ARM-F2-READONLY-RESET' >&2; exit 4; }
tmux new-session -d -s "$SESSION" \
  "'$TICKET/scripts/45-capture-papers3-readonly-reconnect.py' --execute --confirm CAPTURE-F2-READONLY-RESET --duration 75 --output '$RAW' >'$CONSOLE' 2>&1"
