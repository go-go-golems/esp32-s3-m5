...
EXP="$TICKET/scripts/experiments/EXP-20260715-009-factory-f1-dynamic-density"
...
while (($#)); do
  case "$1" in
    ...
    *) echo "usage: $0 [--check|--execute --confirm RUN-DENS-F1]" >&2; exit 2 ;;
  esac
done
...
[[ "$CONFIRM" == RUN-DENS-F1 ]] || { echo 'execute requires --confirm RUN-DENS-F1' >&2; exit 4; }
