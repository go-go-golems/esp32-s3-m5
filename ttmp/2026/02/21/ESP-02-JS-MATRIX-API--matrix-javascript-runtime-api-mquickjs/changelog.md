...
Uploaded updated JS API guide to reMarkable without forcing overwrite of existing annotated document.

### Upload destination

- `/ai/2026/02/22/ESP-02-JS-MATRIX-API/JS-API-GUIDE`
...
Added boot UX features to `0067` firmware:
- boot animation at startup,
- Wi-Fi IP scrolling preview displayed on IP acquisition,
- preview remains until the first user animation starts.
...
Added `matrix fliph on|off` console command (alias for horizontal orientation toggle), and updated matrix console usage/examples accordingly.
...
Added sdkconfig-driven defaults for matrix orientation:
- `CONFIG_TUTORIAL_0067_MATRIX_DEFAULT_FLIPH`
- `CONFIG_TUTORIAL_0067_MATRIX_DEFAULT_FLIPV`

These are now applied during `matrix_engine_init()` at startup.
...
Fixed horizontal flip behavior and added explicit rotate-180 orientation flag.

### Included changes

- `fliph` now performs true full-width horizontal mirroring (module order + intra-module bit reversal).
- Added independent `rot180`/`rotate180` flag and command support.
- Added `rotate_180` to matrix status surfaces (HTTP + JS + console status output).
- Added sdkconfig default for rotate180 (`CONFIG_TUTORIAL_0067_MATRIX_DEFAULT_ROTATE_180`).
