---
Title: Project report and field guide
...
Owners: []
RelatedFiles:
    - Path: 0079-papers3-wamr-assemblyscript-console/README.md
      Note: |-
        Project-local entrypoint and build instructions for the firmware experiment
        Project-local entrypoint and reader-friendly starting point for the report
    - Path: 0079-papers3-wamr-assemblyscript-console/main/papers3_canvas.cpp
      Note: |-
        Display abstraction used by replay and referenced throughout the theory sections
        Display abstraction and replay target used by the Wasm host queue
    - Path: 0079-papers3-wamr-assemblyscript-console/main/wasm_command.cpp
      Note: |-
        Console command surface that exposes the main operator workflows discussed in the report
        Operator-facing console surface and command taxonomy described in the report
    - Path: 0079-papers3-wamr-assemblyscript-console/main/wasm_host_api.cpp
      Note: |-
        Host ABI queue design and replay boundary where the display failure surfaces
        Queued host ABI implementation and replay boundary referenced throughout the debugging narrative
    - Path: 0079-papers3-wamr-assemblyscript-console/main/wasm_module_runner.cpp
      Note: |-
        Core execution path and later execution-state instrumentation
        Main WAMR execution flow
    - Path: 0079-papers3-wamr-assemblyscript-console/main/wasm_replay_control.cpp
      Note: |-
        WAMR-free replay baseline that clarified the debugging story
        Host-only control-path baseline that anchored the investigation
    - Path: 0079-papers3-wamr-assemblyscript-console/tools/build-wasm-demos.mjs
      Note: |-
        Host-side AssemblyScript build pipeline for embedded demo assets
        AssemblyScript compilation pipeline discussed in the architecture sections
ExternalSources:
...
Summary: Long-form project report for the PaperS3 WAMR AssemblyScript console experiment, combining architecture, chronology, wrong turns, debugging theory, and future guidance.
LastUpdated: 2026-03-22T19:35:00-04:00
WhatFor: Preserve the full technical story of the project in a form that is enjoyable and useful to reread years later.
WhenToUse: Read this when you want the full project narrative rather than a narrow postmortem or a ticket diary.
---
