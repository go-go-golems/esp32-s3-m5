...
static uint32 s_wamr_linear_memory_log_budget = 16;
...
wasm_deallocate_linear_memory(WASMMemoryInstance *memory_inst)
{
    ...
    if (s_wamr_linear_memory_log_budget) {
        s_wamr_linear_memory_log_budget--;
        log_this_call = true;
        os_printf("wamr_linear.stage=munmap-enter\n"
                  "wamr_linear.memory_inst=%p\n"
                  "wamr_linear.memory_data=%p\n"
                  "wamr_linear.memory_data_size=%u\n"
                  "wamr_linear.cur_pages=%u\n"
                  "wamr_linear.max_pages=%u\n"
                  "wamr_linear.map_size=%u\n",
                  memory_inst, memory_inst->memory_data,
                  (unsigned)memory_inst->memory_data_size,
                  memory_inst->cur_page_count, memory_inst->max_page_count,
                  (unsigned)map_size);
    }
    ...
    if (log_this_call) {
        os_printf("wamr_linear.stage=munmap-exit\n"
                  "wamr_linear.memory_inst=%p\n",
                  memory_inst);
    }
                            ...
                            uint64 *memory_data_size)
{
    ...
    *memory_data_size = align_as_and_cast(*memory_data_size, page_size);
    ...
    if (s_wamr_linear_memory_log_budget) {
        s_wamr_linear_memory_log_budget--;
        log_this_call = true;
        os_printf("wamr_linear.stage=mmap-enter\n"
                  "wamr_linear.is_shared=%d\n"
                  "wamr_linear.is_memory64=%d\n"
                  "wamr_linear.num_bytes_per_page=%u\n"
                  "wamr_linear.init_pages=%u\n"
                  "wamr_linear.max_pages=%u\n"
                  "wamr_linear.map_size=%u\n"
                  "wamr_linear.memory_data_size=%u\n"
                  "wamr_linear.page_size=%u\n",
                  is_shared_memory, is_memory64,
                  (unsigned)num_bytes_per_page, (unsigned)init_page_count,
                  (unsigned)max_page_count, (unsigned)map_size,
                  (unsigned)*memory_data_size, (unsigned)page_size);
    }
    ...
    if (map_size > 0) {
        ...
        if (!(*data = malloc_func(Alloc_For_LinearMemory,
                                  ...
                                  *memory_data_size))) {
            if (log_this_call) {
                os_printf("wamr_linear.stage=mmap-failed\n");
            }
            return BHT_ERROR;
        }
#else
    ...
    if (log_this_call) {
        os_printf("wamr_linear.stage=mmap-exit\n"
                  "wamr_linear.data=%p\n",
                  *data);
    }
