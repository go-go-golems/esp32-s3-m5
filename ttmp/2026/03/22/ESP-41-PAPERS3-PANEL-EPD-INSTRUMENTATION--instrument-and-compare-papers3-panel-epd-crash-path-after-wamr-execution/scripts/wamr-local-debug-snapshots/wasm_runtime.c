...
{
    uint32 i;
    static uint32 s_wamr_memory_deinit_log_budget = 16;
    if (memories) {
        for (i = 0; i < count; i++) {
            if (memories[i]) {
#if WASM_ENABLE_MULTI_MODULE != 0
                ...
                if (memories[i]->heap_handle) {
                    if (s_wamr_memory_deinit_log_budget) {
                        s_wamr_memory_deinit_log_budget--;
                        os_printf("wamr_mem.stage=destroy-heap\n"
                                  "wamr_mem.index=%u\n"
                                  "wamr_mem.memory_data=%p\n"
                                  "wamr_mem.heap_handle=%p\n",
                                  i, memories[i]->memory_data,
                                  memories[i]->heap_handle);
                    }
                ...
                if (memories[i]->memory_data) {
                    if (s_wamr_memory_deinit_log_budget) {
                        s_wamr_memory_deinit_log_budget--;
                        os_printf("wamr_mem.stage=deallocate-linear-memory\n"
                                  "wamr_mem.index=%u\n"
                                  "wamr_mem.memory_data=%p\n"
                                  "wamr_mem.memory_data_size=%u\n"
                                  "wamr_mem.cur_pages=%u\n"
                                  "wamr_mem.max_pages=%u\n",
                                  i, memories[i]->memory_data,
                                  (unsigned)memories[i]->memory_data_size,
                   ...
                   char *error_buf, uint32 error_buf_size)
{
    ...
    static uint32 s_wamr_memory_init_log_budget = 16;
#if WASM_ENABLE_SHARED_MEMORY != 0
    ...
    if (s_wamr_memory_init_log_budget) {
        s_wamr_memory_init_log_budget--;
        os_printf("wamr_mem.stage=instantiate-memory\n"
                  "wamr_mem.index=%u\n"
                  "wamr_mem.memory_data=%p\n"
                  "wamr_mem.memory_data_size=%u\n"
                  "wamr_mem.num_bytes_per_page=%u\n"
                  "wamr_mem.init_pages=%u\n"
                  "wamr_mem.max_pages=%u\n"
                  "wamr_mem.heap_offset=%u\n"
                  "wamr_mem.heap_size=%u\n",
                  memory_idx, memory->memory_data,
                  (unsigned)memory->memory_data_size, num_bytes_per_page,
                  init_page_count, max_page_count, (unsigned)heap_offset,
                  heap_size);
    }
                 ...
                 uint32 error_buf_size)
{
    ...
    if (s_wamr_instantiate_trace_budget) {
        s_wamr_instantiate_trace_budget--;
        log_this_call = true;
        os_printf("wamr_inst.stage=enter\n"
                  "wamr_inst.module=%p\n"
                  "wamr_inst.parent=%p\n"
                  "wamr_inst.exec_env_main=%p\n"
                  "wamr_inst.stack_size=%u\n"
                  "wamr_inst.heap_size=%u\n"
                  "wamr_inst.max_memory_pages=%u\n",
                  module, parent, exec_env_main, stack_size, heap_size,
                  max_memory_pages);
    }
    ...
    if (!(module_inst =
              runtime_malloc(total_size, error_buf, error_buf_size))) {
        if (log_this_call) {
            os_printf("wamr_inst.stage=alloc-module-inst-failed\n"
                      "wamr_inst.total_size=%u\n"
                      "wamr_inst.error=%s\n",
                      (unsigned)total_size, error_buf ? error_buf : "<null>");
        }
        return NULL;
    }

    if (log_this_call) {
        os_printf("wamr_inst.stage=alloc-module-inst-ok\n"
                  "wamr_inst.total_size=%u\n"
                  "wamr_inst.module_inst=%p\n"
                  "wamr_inst.extra_info_offset=%u\n",
                  (unsigned)total_size, module_inst, extra_info_offset);
    }
    ...
    if ((module_inst->memory_count > 0
        ...
        if (log_this_call) {
            os_printf("wamr_inst.stage=subinstantiate-failed\n"
                      "wamr_inst.module_inst=%p\n"
                      "wamr_inst.memory_count=%u\n"
                      "wamr_inst.table_count=%u\n"
                      "wamr_inst.function_count=%u\n"
                      "wamr_inst.error=%s\n",
                      module_inst, module_inst->memory_count,
                      module_inst->table_count, module_inst->e->function_count,
                      error_buf ? error_buf : "<null>");
        }
        goto fail;
    }
    if (log_this_call) {
        os_printf("wamr_inst.stage=subinstantiate-ok\n"
                  "wamr_inst.module_inst=%p\n"
                  "wamr_inst.memories=%p\n"
                  "wamr_inst.tables=%p\n"
                  "wamr_inst.functions=%p\n",
                  module_inst, module_inst->memories, module_inst->tables,
    ...
    if (log_this_call) {
        os_printf("wamr_inst.stage=success\n"
                  "wamr_inst.module_inst=%p\n"
                  "wamr_inst.default_memory=%p\n"
                  "wamr_inst.exec_env_singleton=%p\n",
                  module_inst,
                  module_inst->memory_count > 0 ? module_inst->memories[0]
                                                : NULL,
                  module_inst->exec_env_singleton);
    }
...
fail:
    if (log_this_call) {
        os_printf("wamr_inst.stage=fail\n"
                  "wamr_inst.module_inst=%p\n"
                  "wamr_inst.error=%s\n",
                  module_inst, error_buf ? error_buf : "<null>");
    }
...
wasm_deinstantiate(WASMModuleInstance *module_inst, bool is_sub_inst)
{
    ...
    if (s_wamr_deinstantiate_trace_budget) {
        s_wamr_deinstantiate_trace_budget--;
        log_this_call = true;
        os_printf("wamr_deinst.stage=enter\n"
                  "wamr_deinst.module_inst=%p\n"
                  "wamr_deinst.is_sub_inst=%d\n"
                  "wamr_deinst.memory_count=%u\n"
                  "wamr_deinst.memories=%p\n"
                  "wamr_deinst.exec_env_singleton=%p\n",
                  module_inst, is_sub_inst, module_inst->memory_count,
                  module_inst->memories, module_inst->exec_env_singleton);
    }
    ...
    if (log_this_call) {
        os_printf("wamr_deinst.stage=free-module-inst\n"
                  "wamr_deinst.module_inst=%p\n",
                  module_inst);
    }
