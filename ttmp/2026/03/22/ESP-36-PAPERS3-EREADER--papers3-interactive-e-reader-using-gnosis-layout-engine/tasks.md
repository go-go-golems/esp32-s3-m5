...
- [ ] Phase 1: Create EReaderApp skeleton with InitBoard, hardcoded page buffer, reading screen using ext_text, verify text renders on EPD
- [ ] Phase 2: Implement BookStore - SPIFFS mount, books.idx parsing, ReadChunk file I/O, create spiffs_data/ with sample book
- [ ] Phase 3: Implement Paginator - word-wrap algorithm, page offset table, FormatPageBuffer with newline insertion
- [ ] Phase 3: Wire pagination into reading screen - NextPage/PreviousPage update page buffer and mark dirty, verify partial EPD refresh
- [ ] Phase 4: Implement touch navigation - reading screen touch zones (left 25% prev, right 25% next), status bar updates (page number, progress bar)
