...
- [x] Task 2.2: Run `empty-module`, `return-42`, and `return-42 binary_freeable` direct-embedded loads on AtomS3R with the same bounded WAMR const-string trace.
- [x] Task 2.3: Compare whether AtomS3R takes the same in-place rewrite path and whether later PSRAM touch survives it.
- [x] Task 3: Write an interpreted comparison, not just a raw diff.
- [ ] Task 4: Keep the diary current and store any comparison scripts in this ticket’s `scripts/` directory.
- [x] Task 5: Reconcile the earlier board-specific theory with the new Atom result and identify what still remains board-specific versus runtime-generic.
