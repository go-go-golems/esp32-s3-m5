...
- [ ] Task 1: Write the embedded-buffer investigation plan and carry forward the precise `ESP-42` boundary into this ticket.
- [ ] Task 2: Map how the embedded Wasm assets are linked and surfaced to runtime in `0082`.
- [ ] Task 2.1: Inspect `wasm_module_registry.cpp`, linker-generated symbols, and resulting pointer ranges for the embedded assets.
- [ ] Task 2.2: Record whether the embedded bytes live in flash-mapped IROM/DROM space, another mapped region, or a copied RAM region.
- [ ] Task 3: Compare the embedded-buffer path against explicit RAM-copy paths at the code level.
- [ ] Task 3.1: Trace the exact `wasm_runtime_load(...)` call inputs for embedded, copied-internal, and copied-spiram modes.
- [ ] Task 3.2: Record what actually differs besides pointer locality.
