...
#ifdef ESP_PLATFORM
static uint32 g_wamr_loader_probe_budget = 32;
                        ...
                        uint32 size)
{
    ...
    printf("wamr_loader.stage=%s\n"
           ...
           "wamr_loader.spiram_free=%u\n"
           "wamr_loader.module=%p\n"
           "wamr_loader.module_external=%s\n"
           "wamr_loader.ptr=%p\n"
           ...
           module != NULL && esp_ptr_external_ram((void *)module) ? "yes" : "no",
           ptr, ptr != NULL && esp_ptr_external_ram((void *)ptr) ? "yes" : "no",
           (unsigned)size);
}
#endif
...
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("load-enter", module, buf, size);
#endif
...
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("load-before-create-sections", module, p,
                            (uint32)(p_end - p));
#endif
...
#ifdef ESP_PLATFORM
        print_wamr_loader_probe("load-fail", module, section_list, size);
#endif
...
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("load-after-load-from-sections", module, section_list,
                            size);
#endif
    destroy_sections(section_list);
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("load-exit-ok", module, buf, size);
#endif
    return true;
fail:
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("load-fail", module, p, (uint32)(p_end - p));
#endif
    return false;
}
                 ...
                 const LoadArgs *args, char *error_buf, uint32 error_buf_size)
{
...
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("loader-create-module-ok", module, buf, size);
#endif
    ...
    if (!load(buf, size, module, args->wasm_binary_freeable, args->no_resolve,
              error_buf, error_buf_size)) {
#ifdef ESP_PLATFORM
        print_wamr_loader_probe("loader-load-fail", module, buf, size);
#endif
        goto fail;
    }
    ...
    if (!check_wasi_abi_compatibility(module,
                                      ...
                                      error_buf, error_buf_size)) {
#ifdef ESP_PLATFORM
        print_wamr_loader_probe("loader-wasi-check-fail", module, buf, size);
#endif
        goto fail;
    }
#endif
...
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("loader-exit-ok", module, buf, size);
#endif
...
fail:
#ifdef ESP_PLATFORM
    print_wamr_loader_probe("loader-unload-on-fail", module, buf, size);
#endif
    wasm_loader_unload(module);
    return NULL;
}
