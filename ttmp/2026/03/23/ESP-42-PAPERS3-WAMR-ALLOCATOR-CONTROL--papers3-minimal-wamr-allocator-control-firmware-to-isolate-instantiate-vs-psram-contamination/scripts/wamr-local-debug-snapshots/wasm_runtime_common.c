...
#ifdef ESP_PLATFORM
static uint32 g_wamr_runtime_load_probe_budget = 16;
                              ...
                              const void *module_common)
{
    ...
    printf("wamr_rt_load.stage=%s\n"
           ...
           "wamr_rt_load.spiram_free=%u\n"
           "wamr_rt_load.buf=%p\n"
           "wamr_rt_load.buf_external=%s\n"
           "wamr_rt_load.size=%u\n"
           ...
           buf != NULL && esp_ptr_external_ram((void *)buf) ? "yes" : "no",
           (unsigned)size, module_common,
           module_common != NULL && esp_ptr_external_ram((void *)module_common)
               ? "yes"
               : "no");
}
#endif
...
#ifdef ESP_PLATFORM
    print_wamr_runtime_load_probe("enter", buf, size, NULL);
#endif
    WASMModuleCommon *module_common =
        wasm_runtime_load_ex(buf, size, &args, error_buf, error_buf_size);
#ifdef ESP_PLATFORM
    print_wamr_runtime_load_probe(module_common ? "exit-ok" : "exit-fail", buf,
                                  size, module_common);
#endif
    return module_common;
}
...
wasm_runtime_instantiate_internal(WASMModuleCommon *module,
                                  ...
                                  char *error_buf, uint32 error_buf_size)
{
    static uint32 s_wamr_runtime_instantiate_log_budget = 16;
    bool log_this_call = false;

    if (s_wamr_runtime_instantiate_log_budget) {
        s_wamr_runtime_instantiate_log_budget--;
        log_this_call = true;
        os_printf("wamr_rt.stage=instantiate-enter\n"
                  "wamr_rt.module=%p\n"
                  "wamr_rt.parent=%p\n"
                  "wamr_rt.exec_env_main=%p\n"
                  "wamr_rt.stack_size=%u\n"
                  "wamr_rt.heap_size=%u\n"
                  "wamr_rt.max_memory_pages=%u\n",
    ...
    if (module->module_type == Wasm_Module_Bytecode) {
        WASMModuleInstanceCommon *module_inst =
            (WASMModuleInstanceCommon *)wasm_instantiate(
            (WASMModule *)module, (WASMModuleInstance *)parent, exec_env_main,
            stack_size, heap_size, max_memory_pages, error_buf, error_buf_size);
        if (log_this_call) {
            os_printf("wamr_rt.stage=instantiate-exit\n"
                      "wamr_rt.module_inst=%p\n"
                      "wamr_rt.error=%s\n",
    ...
    if (module->module_type == Wasm_Module_AoT) {
        WASMModuleInstanceCommon *module_inst =
            (WASMModuleInstanceCommon *)aot_instantiate(
    ...
    if (log_this_call) {
        os_printf("wamr_rt.stage=instantiate-invalid-module-type\n"
                  "wamr_rt.error=%s\n",
...
wasm_runtime_deinstantiate_internal(WASMModuleInstanceCommon *module_inst,
                                    bool is_sub_inst)
{
    static uint32 s_wamr_runtime_deinstantiate_log_budget = 16;
    bool log_this_call = false;

    if (s_wamr_runtime_deinstantiate_log_budget) {
        s_wamr_runtime_deinstantiate_log_budget--;
        log_this_call = true;
        os_printf("wamr_rt.stage=deinstantiate-enter\n"
                  "wamr_rt.module_inst=%p\n"
                  "wamr_rt.is_sub_inst=%d\n"
                  "wamr_rt.module_type=%u\n",
                  module_inst, is_sub_inst,
                  module_inst ? module_inst->module_type : UINT32_MAX);
    }
    ...
    if (module_inst->module_type == Wasm_Module_Bytecode) {
        ...
        if (log_this_call) {
            os_printf("wamr_rt.stage=deinstantiate-exit\n"
                      "wamr_rt.module_inst=%p\n",
                      module_inst);
        }
        return;
