...
- [x] Decide whether touch is part of v1 UI interaction; if yes, register an LVGL pointer input device and map screen coordinates correctly
- [x] Implement `main/timer_model.h` and `main/timer_model.cpp` as a small non-blocking countdown engine with `idle`, `running`, `paused`, and `complete` states
- [x] Reuse the `esp_timer_get_time()`-based timing model from `0071` instead of any blocking countdown logic
- [x] Implement `main/ui_timer_screen.h` and `main/ui_timer_screen.cpp` with a large central timer label, progress arc, status label, and minimal action controls
- [x] Choose the v1 visual palette and typography so the UI feels intentional on the round screen rather than like a rectangular layout cropped into a circle
- [x] Implement `main/timer_controller.h` and `main/timer_controller.cpp` to map encoder and touch actions into timer-model actions
...
- [ ] Reduce visible UI tearing by enabling LVGL double buffering and avoiding unnecessary screen invalidation
  Note: this was prototyped in Step 7 and reverted in Step 8 at the user's request because the RAM and update-style tradeoff was not desired for the current demo.
...
- [x] Flash the project to real M5Dial hardware and verify boot, display, encoder, press, touch, and reset behavior
  Note: boot, display, encoder, button/reset, and touch-swipe behavior are now verified on `/dev/ttyACM0`; the separate tearing-mitigation experiment was later reverted.
...
- [x] Add a short operator-facing README section with exact build, flash, and monitor commands for the new tutorial
