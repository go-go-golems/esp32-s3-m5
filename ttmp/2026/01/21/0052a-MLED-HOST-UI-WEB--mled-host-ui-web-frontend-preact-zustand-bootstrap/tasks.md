...
- [ ] Implement Patterns screen: Preview/Save/Delete actions
- [ ] Implement Status screen: SystemBanner with health summary (Healthy/Degraded/Offline)
- [ ] Implement Status screen: ProblemList with auto-detection (weak signal, offline nodes)
- [ ] Implement Status screen: NodeTable with sortable columns (name, signal, uptime, pattern, status)
- [ ] Implement Settings modal/page: network config (bind IP, multicast), timing, import/export presets
- [x] Implement SSE event handling for live node updates (node.update, node.offline, apply.ack)
- [ ] Add default presets (Rainbow, Calm, Warm, Pulse, Off) with proper config structures
