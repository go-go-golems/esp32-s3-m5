...
- [ ] Capture a complete failing boot log (include `app_init` “App version” + SHA + build timestamp)
- [ ] Prove we are flashing the intended binary:
...
- [ ] Add boot-time instrumentation (minimal, before the first `xQueueReceive`):
  - [ ] `components/mqjs_service/mqjs_service.cpp`: log `s`, `s->q`, `s->ready`, core id, task name/priority
