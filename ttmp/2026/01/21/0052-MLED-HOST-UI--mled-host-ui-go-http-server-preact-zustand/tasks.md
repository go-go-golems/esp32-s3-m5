...
- [x] Create `esp32-s3-m5/mled-server/` Go module + `cmd/mled-server` entrypoint.
- [x] Implement MLED/1 codec (header + payload pack/unpack) matching `2026-01-21--mled-web/c/mledc/protocol.h`.
- [x] Implement u32 show-time helpers (wrap-around-safe diff/duration/isDue) matching `2026-01-21--mled-web/c/mledc/time.c`.
- [x] Implement UDP multicast transport:
  - bind local port (default `:4626`)
  - join multicast group (default `239.255.32.6`)
  - set TTL=1
  - select outbound interface/bind IP
- [x] Implement controller epoch/show clock:
  - generate `epoch_id` at startup
  - `msg_id` counter
  - `show_ms()` based on monotonic clock, masked to u32
- [x] Implement receive loop that:
...
- [x] Implement node status tracking:
...
- [x] Implement “apply” operation as a protocol-compatible mapping:
...
- [x] Implement HTTP server (default `localhost:8765`):
...
- [x] Implement WebSocket server at `GET /ws` sending events:
...
- [x] Implement minimal persistence:
  - presets JSON file in configurable `--data-dir` (default `./var/`)
  - settings JSON file (same dir)

### Go: validation
- [x] Unit tests for codec sizes and sample pack/unpack round-trips.
- [x] Unit tests for u32 time helper behavior around wrap-around.
- [x] Local smoke plan:
...
- [x] Scaffold Go module under esp32-s3-m5/mled-server
- [x] Implement MLED/1 codec + u32 show-time helpers (match mledc/protocol.h + time.c)
- [x] Implement UDP engine (multicast join, PONG/ACK/TIME_REQ handling, node cache + offline sweeper)
- [x] Implement HTTP API + WebSocket events per imported UI spec (/api/* and /ws)
