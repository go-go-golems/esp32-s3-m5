...
- [ ] Create component skeleton at `esp32-s3-m5/components/wifi_mgr/` (CMakeLists, Kconfig, include/)
- [ ] Copy baseline implementation from `esp32-s3-m5/0048-cardputer-js-web/main/wifi_mgr.c` into `components/wifi_mgr/wifi_mgr.c`
- [ ] Standardize Kconfig: introduce `CONFIG_WIFI_MGR_MAX_RETRY` and `CONFIG_WIFI_MGR_AUTOCONNECT_ON_BOOT`
- [ ] Replace config references in component code (`CONFIG_TUTORIAL_0048_WIFI_*` → `CONFIG_WIFI_MGR_*`)
- [ ] Migrate `esp32-s3-m5/0046-xiao-esp32c6-led-patterns-webui/` to component
- [ ] Migrate `esp32-s3-m5/0048-cardputer-js-web/` to component
