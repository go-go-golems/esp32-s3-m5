#!/usr/bin/env python3
"""Probe the 40x20 PicoOS home launcher layout and app registry visibility."""
...
def main():
 ...
 try:
  ...
  apps=send(fd,'screen eval /apps',args.timeout)+send(fd,'screen dump',args.timeout)
  home=send(fd,'picoos launcher',args.timeout)+send(fd,'picojs dump',args.timeout)
  checks=[
   'APPS: ESP_OK COUNT=10' in apps,
   all(x in apps for x in ['*home','*repl','hello','sysmon','snake','calc','settings','notes']),
   'music' not in apps and 'chat' not in apps,
   'PicoOS' in home and 'Apps' in home,
   all(x in home for x in ['repl','hello','sysmon','snake','calc','settings','notes']),
   'select repl | enter open | brk repl' in home,
  ]
  ok=all(checks); print('HOME_LAUNCHER_PROBE','PASS' if ok else 'FAIL',checks); return 0 if ok else 1
 finally:
