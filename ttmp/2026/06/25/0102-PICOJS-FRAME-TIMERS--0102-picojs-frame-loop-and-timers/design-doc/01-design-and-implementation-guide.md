...
ExternalSources: []
Summary: Deterministic frames, timers, loops, compute callbacks, and callback error handling.
LastUpdated: 2026-06-25T15:30:00-07:00
WhatFor: Plan and implementation guide for support time-based apps without giving JavaScript direct control over the FreeRTOS scheduler.
...
This child ticket is Phase 6 of the umbrella `0102-PICOJS-DEVICE-INTEGRATION` effort. Its purpose is to support time-based apps without giving JavaScript direct control over the FreeRTOS scheduler. The phase must end with a buildable firmware tree, explicit console-visible validation, diary entries, and focused commits.
...
- Implement `app.on("tick", ms, fn)`.
- Implement `app.loop(fps, fn)`.
- Implement `app.compute(fn)`.
- Add `picojs run <count> <dt_ms>` for deterministic console-driven progression.
- Track callback errors and last frame status.
...
- Frame advancement is host-controlled; user scripts do not own a blocking loop.
- Timer callbacks execute on the QuickJS service task.
- One bad callback should produce a visible error and a recoverable state.
...
- Counters/timers advance deterministically through console commands.
- Loop callbacks run the expected number of times for a given dt.
- Thrown callback errors are captured and reported.
