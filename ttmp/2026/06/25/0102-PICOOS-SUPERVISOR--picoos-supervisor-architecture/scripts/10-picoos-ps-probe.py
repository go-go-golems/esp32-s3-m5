#!/usr/bin/env python3
"""Probe PicoOS process/status views on UART and visual REPL."""
...
def main():
 ...
 try:
  ...
  cli=send(fd,'picoos ps',args.timeout)
  lcd=send(fd,'screen eval /ps',args.timeout)+send(fd,'screen dump',args.timeout)
  checks=['picoos ps: ESP_OK count=10' in cli,'state=' in cli,'PS: ESP_OK COUNT=10' in lcd, all(x in lcd for x in ['home','repl','snake','calc','settings','notes'])]
  ok=all(checks); print('PICOOS_PS_PROBE','PASS' if ok else 'FAIL',checks); return 0 if ok else 1
 finally:
