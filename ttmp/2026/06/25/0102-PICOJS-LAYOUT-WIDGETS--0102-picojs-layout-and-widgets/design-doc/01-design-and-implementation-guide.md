...
This child ticket is Phase 4 of the umbrella `0102-PICOJS-DEVICE-INTEGRATION` effort. Its purpose is to expand the native DSL from a single panel into useful dashboard layouts. The phase must end with a buildable firmware tree, explicit console-visible validation, diary entries, and focused commits.
...
- Implement `App.layout(fn)` with `Layout.row` and `Layout.col` builder methods.
- Bind panels to named regions.
- Implement gauge widgets and dynamic value evaluation.
- Add a dashboard example load path and console validation.
...
- Layout builder callbacks are synchronous; scripts must not store the temporary layout object.
- Single-axis layout is enough for the first dashboard slice.
- Gauge value callbacks must report exceptions without crashing the frame.
...
- Dashboard dump shows separate bar/body regions.
- Gauge values update when frames advance.
- Callback exceptions are visible in `picojs status` or console output.
