...
The PicoCalc LCD backlight is driven by a PWM-controlled LED driver circuit on the PicoCalc mainboard. The full same-position adapter pin map is documented in the `ESP32-P4-PICOCALC` ticket design doc `03-full-rpico-socket-to-waveshare-esp32-p4-pin-map.md`. On the original RP2040 PicoCalc, the backlight PWM was generated by RP2040 GP12. On the newer PicoCalc hardware with the STM32 southbridge, the STM32 generates the backlight PWM signal and exposes brightness control via I2C register.
