...
- [ ] Manual typing confirmation (no tmux automation): attach to the running monitor and type `1+2<enter>`; record whether any bytes echo or evaluate.
- [ ] Bypass `idf_monitor`: run `imports/test_storage_repl.py` against `localhost:5555` and capture its stdout; determine whether *any* RX reaches `uart_read_bytes()`.
