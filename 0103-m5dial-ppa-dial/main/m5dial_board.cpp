...
void M5DialBoard::poll_touch(QueueHandle_t event_queue) {
  ...
  // Release without a swipe counts as a tap; PPA Dial treats it like the
  // encoder button (prototype: any touch activates the scene).
  if (!touch_swipe_reported_) {
    post_event(event_queue, InputEvent{InputEventType::kButtonShortPress, 1, SwipeDirection::kNone, now_us()});
  }
