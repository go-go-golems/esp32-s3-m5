/* js_command.cpp — native QuickJS console commands for 0101.
 *
 * Supported commands:
 *   js status
 *   js eval <source>
...
namespace {
constexpr const char *kTag = "0101_js";
...
int cmd_js(int argc, char **argv)
{
    ...
    if (std::strcmp(argv[1], "eval") == 0) {
        ...
        return run_eval("console-eval", buf, kDefaultEvalTimeoutMs);
    }
...
void register_js_commands(qjs_service_t *svc)
{
    ...
    ESP_LOGI(kTag, "registered native QuickJS console commands");
}
