...
static const char* TAG = "chain_enc_0071";
...
uint8_t crc8_sum(const uint8_t* p, size_t n) {
  uint8_t s = 0;
...

  {
    ...
    while (xTaskGetTickCount() < deadline) {
      ...
      if (f.cmd == kCmdGetDeviceType && f.data.size() >= 2) {
        ...
        ESP_LOGI(TAG, "device_type=0x%04x", (unsigned)t);
        break;
      }
    }
  }
...
void ChainEncoderUart::build_frame(uint8_t cmd, const uint8_t* data, size_t data_len, std::vector<uint8_t>* out) const {
  ...
  const uint16_t payload_len = (uint16_t)(3 + data_len);
  ...
  const uint8_t crc = crc8_sum(out->data() + 4, (size_t)(payload_len - 1));
  out->push_back(crc);
