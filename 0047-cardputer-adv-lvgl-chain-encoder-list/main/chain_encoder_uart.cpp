...
static const char *TAG = "chain_enc";
...
void ChainEncoderUart::task_main() {
    ESP_LOGI(TAG, "start: uart=%d baud=%d tx=%d rx=%d index=%u poll_ms=%d", cfg_.uart_num, cfg_.baud, cfg_.tx_gpio,
             cfg_.rx_gpio, (unsigned)cfg_.index_id, cfg_.poll_ms);
...
bool ChainEncoderUart::read_frame(Frame *out, TickType_t deadline_ticks) {
    ...
    // First try extracting from existing buffer.
    if (try_extract_one(out)) return true;

    uint8_t tmp[256];
    const TickType_t now = xTaskGetTickCount();
    const TickType_t wait = (deadline_ticks > now) ? (deadline_ticks - now) : 0;
    const int n = uart_read_bytes((uart_port_t)cfg_.uart_num, tmp, sizeof(tmp), wait);
    if (n <= 0) return false;
    feed_bytes(tmp, (size_t)n);
...
void ChainEncoderUart::feed_bytes(const uint8_t *data, size_t n) {
    ...
    if (buf_.size() > 4096) {
        // Keep bounded; drop oldest bytes if we somehow fall behind.
...
bool ChainEncoderUart::try_extract_one(Frame *out) {
    ...
    size_t i = 0;
    while (i + 1 < buf_.size()) {
        if (buf_[i] == kHeadHi && buf_[i + 1] == kHeadLo) break;
        i++;
    }
    if (i > 0) {
        buf_.erase(buf_.begin(), buf_.begin() + (ptrdiff_t)i);
    }
    if (buf_.size() < 6) return false;

    const uint16_t payload_len = (uint16_t)buf_[2] | ((uint16_t)buf_[3] << 8);
    if (payload_len < 3 || payload_len > 256) {
        buf_.erase(buf_.begin()); // resync
        return false;
    }
    const size_t total_len = (size_t)payload_len + 6; // header(2)+len(2)+payload+tail(2)
    if (buf_.size() < total_len) return false;

    if (buf_[total_len - 2] != kTailHi || buf_[total_len - 1] != kTailLo) {
        buf_.erase(buf_.begin()); // resync
        return false;
    }

    const uint8_t crc_in = buf_[4 + payload_len - 1];
    ...
    out->cmd = buf_[5];
    out->data.clear();
    const size_t data_len = (size_t)payload_len - 3; // minus Index/Cmd/CRC
    if (data_len > 0) {
        out->data.insert(out->data.end(), buf_.begin() + 6, buf_.begin() + 6 + (ptrdiff_t)data_len);
    }

    buf_.erase(buf_.begin(), buf_.begin() + (ptrdiff_t)total_len);
    return true;
}

void ChainEncoderUart::handle_unsolicited(const Frame &f) {
    // 0xE0 payload: Status, 0x00 (per PDF). Store trigger status (0,1,2).
    if (f.data.size() < 1) return;
    click_pending_.store((uint8_t)(f.data[0] + 1)); // store non-zero to signal pending
}
