...
constexpr uint32_t kBootCaptureGraceMs = 10000;
constexpr uint32_t kInterphaseSettledMs = 4000;
constexpr uint32_t kTerminalSettledMs = 8000;
...
const char* TargetName(TargetKind target)
{
    switch (target) {
        case TargetKind::kWhite:
            return "white";
        case TargetKind::kFullGrey1:
            return "full-grey-1";
        case TargetKind::kFullGrey2:
            return "full-grey-2";
...
bool PrepareFixture(TargetKind target)
{
    ...
    switch (target) {
        case TargetKind::kWhite:
            return true;
        case TargetKind::kFullGrey1:
            std::memset(g_fixture, 0x55, kPackedBytes);
            return true;
        case TargetKind::kFullGrey2:
            std::memset(g_fixture, 0xAA, kPackedBytes);
            return true;
        case TargetKind::kFullBlack:
        case TargetKind::kArea100:
...
void PrintStepMarker(const char* phase, const char* boundary, TargetKind target, bool result)
{
    char target_hash[65] = {};
    if (target == TargetKind::kNone) {
        std::snprintf(target_hash, sizeof(target_hash), "none");
    } else {
        HashFixture(target_hash);
    }
    std::printf(
        "EPD_DENSITY_STEP schema=esp50.epd-density-step.v1 device_us=%" PRId64
        " phase=%s boundary=%s target=%s target_sha256=%s result=%s pending=%d rails=%s"
        " heap_free=%u heap_min=%u\n",
        esp_timer_get_time(), phase, boundary, TargetName(target), target_hash,
        result ? "ok" : "fail", g_display.pendingStages(),
        g_display.panelPowerActive() ? "active" : "idle",
        static_cast<unsigned>(heap_caps_get_free_size(MALLOC_CAP_8BIT)),
        static_cast<unsigned>(heap_caps_get_minimum_free_size(MALLOC_CAP_8BIT)));
    std::fflush(stdout);
}

bool RunStep(const char* phase, OperationKind kind, TargetKind target, uint32_t settled_ms)
{
    if (!PrepareFixture(target)) {
        PrintStepMarker(phase, "fixture-failed", target, false);
        g_state = ControlState::kFault;
        return false;
    }
    PrintStepMarker(phase, "begin", target, true);
    const bool result = RunOperation(kind, target, phase);
    PrintStepMarker(phase, "idle", target, result);
    if (!result || g_display.pendingStages() != 0 || g_display.panelPowerActive()) {
        PrintStepMarker(phase, "post-idle-invariant-failed", target, false);
        g_state = ControlState::kFault;
        return false;
    }
    vTaskDelay(pdMS_TO_TICKS(settled_ms));
    PrintStepMarker(phase, "settled", target, true);
    return true;
}

void DensityStepResponseTask(void*)
{
    PrintStepMarker("boot", "capture-grace-begin", TargetKind::kNone, true);
    vTaskDelay(pdMS_TO_TICKS(kBootCaptureGraceMs));
    PrintStepMarker("boot", "capture-grace-end", TargetKind::kNone, true);
    const bool ok = RunStep("cleanup-white", OperationKind::kCleanup, TargetKind::kWhite,
                            kInterphaseSettledMs) &&
                    RunStep("full-grey-1", OperationKind::kPaint, TargetKind::kFullGrey1,
                            kInterphaseSettledMs) &&
                    RunStep("full-grey-2", OperationKind::kPaint, TargetKind::kFullGrey2,
                            kInterphaseSettledMs) &&
                    RunStep("full-black", OperationKind::kPaint, TargetKind::kFullBlack,
                            kInterphaseSettledMs) &&
                    RunStep("full-white", OperationKind::kPaint, TargetKind::kWhite,
                            kTerminalSettledMs);
    PrintStepMarker("experiment", ok ? "end" : "fault", TargetKind::kNone, ok);
    vTaskDelete(nullptr);
}
...
extern "C" void app_main(void)
{
    ...
    PrintStatus();
    if (g_state == ControlState::kBootLocked &&
        xTaskCreatePinnedToCore(DensityStepResponseTask, "epd_density_step", 6144, nullptr, 8, nullptr, 1) == pdPASS) {
        ESP_LOGI(kTag, "automatic density step response armed; panel work begins after %u ms",
                 static_cast<unsigned>(kBootCaptureGraceMs));
    } else {
        ESP_LOGE(kTag, "automatic density step response unavailable; panel commands remain disabled");
    }
}
