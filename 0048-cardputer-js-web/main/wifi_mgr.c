/*
 * 0048: Wi-Fi STA manager (console-friendly).
 *
 * Adapted from: 0046-xiao-esp32c6-led-patterns-webui/main/wifi_mgr.c
 *
...
static const char* TAG = "wifi_mgr_0048";
...
static void wifi_event_handler(void* arg, esp_event_base_t base, int32_t id, void* data) {
  ...
  if (base == WIFI_EVENT && id == WIFI_EVENT_STA_DISCONNECTED) {
    ...
    const bool do_retry =
        s_autoconnect && creds_present_unsafe() && (s_retry < CONFIG_TUTORIAL_0048_WIFI_MAX_RETRY);
    unlock_mu();
    ...
    if (do_retry) {
      ...
      ESP_LOGE(TAG, "max retries reached (%d)", CONFIG_TUTORIAL_0048_WIFI_MAX_RETRY);
    }
    return;
  }
...
esp_err_t wifi_mgr_start(void) {
  ...
  lock_mu();
  s_autoconnect = CONFIG_TUTORIAL_0048_WIFI_AUTOCONNECT_ON_BOOT && s_has_runtime;
  unlock_mu();

  if (s_has_runtime) {
    ESP_ERROR_CHECK(apply_runtime_config());
    ESP_LOGI(TAG,
             "Wi-Fi ready: %s (ssid=%s)",
             s_autoconnect ? "autoconnect enabled" : "autoconnect disabled",
             s_runtime_ssid);
  } else {
    ESP_LOGW(TAG, "Wi-Fi ready: no credentials yet; configure via esp_console (wifi scan, wifi set, wifi connect)");
  }
