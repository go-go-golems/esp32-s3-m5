...
# One-command build + flash helper for the Cardputer M5GFX demo suite.
#
# Usage:
...
if [[ "${1:-}" == "tmux-flash-monitor" ]]; then
  ...
  session="cardputer-m5gfx-demo-suite"
  # Run flash+monitor in a single pane to avoid serial-port locking conflicts.
  # Second pane is an interactive shell in the project dir (for rebuilds, logs, etc).
