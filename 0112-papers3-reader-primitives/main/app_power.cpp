...
// The retained sleep image (task 078s): what stays on the panel while the
// device is off. E-paper keeps it without power.
StatusCode PresentSleepImage(SleepMode mode, uint32_t seconds) {
    ...
    FillReaderSnapshot(&snapshot);
    if (snapshot.open != 0) {
        char line[s3paper::TextProps::kCapacity];
        snprintf(line, sizeof(line), "%s", snapshot.title);
        (void)a.AddChild(
            content, s3paper::NewText(a, line, s3paper::kFontBody, 0,
                                      s3paper::TextAlign::Center)
                         .value);
        snprintf(line, sizeof(line), "%u.%u%% read",
                 static_cast<unsigned>(snapshot.progress_permille / 10),
                 static_cast<unsigned>(snapshot.progress_permille % 10));
        (void)a.AddChild(
            content, s3paper::NewText(a, line, s3paper::kFontUi, 96,
                                      s3paper::TextAlign::Center)
                         .value);
    }
    (void)a.AddChild(content,
                     s3paper::NewText(a, "asleep", s3paper::kFontBody, 0,
                                      s3paper::TextAlign::Center)
                         .value);
...
void PowerAutoTick(int64_t now_us) {
    // Low-battery shutdown (task tv0p): qualified detection = ADC level
    // AND not charging, sampled at most every 30 s, only after M5 is up
    // (never force an init just to read the battery). Shutdown goes
    // through the same quiesce sequence, so state cannot corrupt.
    static int64_t s_last_batt_check_us = 0;
    if (M5BackendState().initialized &&
    ...
    // Idle since the later of boot and the last touch input; any input
    // restarts the window (the user-cancel path).
