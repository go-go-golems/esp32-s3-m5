...
#include "js_bc_hello.h"  // generated by tools/js/build_bytecode_apps.sh
...
StatusCode JsLoadBytecodeApps() {
    uint8_t *buf = static_cast<uint8_t *>(heap_caps_malloc(
        sizeof(kJsBytecode_bc_hello), MALLOC_CAP_INTERNAL));
    if (buf == nullptr) {
        return StatusCode::OutOfMemory;
    }
    memcpy(buf, kJsBytecode_bc_hello, sizeof(kJsBytecode_bc_hello));
    if (!JS_IsBytecode(buf, sizeof(kJsBytecode_bc_hello)) ||
        JS_RelocateBytecode(s_ctx, buf, sizeof(kJsBytecode_bc_hello)) != 0) {
        free(buf);
        return StatusCode::CorruptData;
    }
...
StatusCode JsRunApp(uint32_t which) {
    ...
    switch (which) {
        ...
        case 9: return JsRunBytecodeHello();
