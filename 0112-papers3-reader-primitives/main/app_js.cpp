...
constexpr uint32_t kMaxJsHits = 48;  // postcard keyboard alone needs ~34
...
// PULP OS interval timer: fires s3Dispatch(100,...) from the owner loop
// while the JS screen is active. 0 = stopped.
uint32_t s_timer_interval_ms = 0;
int64_t s_timer_due_us = 0;
...
StatusCode JsLoadBytecodeApps();  // defined below (needs kJsBytecode_*)
...
const char kFacadeJs[] =
    ...
    "  var api = {\n"
    "    _taps: {}, _nextHit: 1,\n"
    "    FONT_UI: 0, FONT_BODY: 1, FONT_DISPLAY: 2, FONT_XL: 3,\n"
...
StatusCode JsInit() {
    ...
    // Bytecode images must load while the context has no RAM atoms —
    // i.e. before the facade (or anything else) evaluates.
    const StatusCode bc = JsLoadBytecodeApps();
    if (bc != StatusCode::Ok) {
...
// Trusted embedded bytecode (task ibe5/m1w2). MUST run at context setup:
// JS_LoadBytecode requires that no RAM atoms exist yet, so the image is
// relocated and loaded BEFORE the facade evaluates. The buffer outlives
// the context and is never freed.
StatusCode JsLoadBytecodeApps() {
    uint8_t *buf = static_cast<uint8_t *>(heap_caps_malloc(
        sizeof(kJsBytecode_pulp), MALLOC_CAP_INTERNAL));
    if (buf == nullptr) {
        return StatusCode::OutOfMemory;
    }
    memcpy(buf, kJsBytecode_pulp, sizeof(kJsBytecode_pulp));
    if (!JS_IsBytecode(buf, sizeof(kJsBytecode_pulp)) ||
        JS_RelocateBytecode(s_ctx, buf, sizeof(kJsBytecode_pulp)) != 0) {
        free(buf);
        return StatusCode::CorruptData;
    }
    s_bc_main = JS_LoadBytecode(s_ctx, buf);
    if (JS_IsException(s_bc_main)) {
        const JSValue err = JS_GetException(s_ctx);
        printf("js bytecode load failed: ");
        JS_PrintValueF(s_ctx, err, JS_DUMP_LONG);
        printf("\n");
        free(buf);
        return StatusCode::CorruptData;
...
StatusCode JsRunBytecodePulp() {
    if (!s_bc_loaded) {
        return StatusCode::InvalidArgument;
    }
    s_deadline_us = esp_timer_get_time() + 3'000'000;
    const JSValue out = JS_Run(s_ctx, s_bc_main);
    s_deadline_us = 0;
    s_evals++;
    if (JS_IsException(out)) {
        s_exceptions++;
        const JSValue err = JS_GetException(s_ctx);
        printf("js error in <pulp>: ");
        JS_PrintValueF(s_ctx, err, JS_DUMP_LONG);
        printf("\n");
        return StatusCode::CorruptData;
    }
...
StatusCode JsRunApp(uint32_t which) {
    ...
    switch (which) {
        ...
        case 9: return JsRunBytecodePulp();
        default: return StatusCode::InvalidArgument;
    }
}

void JsTimerTick(int64_t now_us) {
    if (s_timer_interval_ms == 0 || s_ctx == nullptr || !JsScreenActive() ||
        now_us < s_timer_due_us) {
        return;
    }
    s_timer_due_us = now_us + static_cast<int64_t>(s_timer_interval_ms) * 1000;
    s_dispatches++;
    (void)EvalBounded("s3Dispatch(100,0,0,0)", 1000, "<timer>");
}
...
JSValue js_s3_timer_start(JSContext *ctx, JSValue *, int argc,
                          JSValue *argv) {
    int interval_ms = 0;
    if (argc < 1 || JS_ToInt32(ctx, &interval_ms, argv[0])) {
        return JS_EXCEPTION;
    }
    if (interval_ms < 250) {
        interval_ms = 250;  // owner-loop tick resolution floor
    }
    s_timer_interval_ms = static_cast<uint32_t>(interval_ms);
    s_timer_due_us = esp_timer_get_time() +
                     static_cast<int64_t>(interval_ms) * 1000;
    return JS_UNDEFINED;
}

JSValue js_s3_timer_stop(JSContext *ctx, JSValue *, int, JSValue *) {
    (void)ctx;
    s_timer_interval_ms = 0;
    return JS_UNDEFINED;
}

JSValue js_s3_store_get(JSContext *ctx, JSValue *, int argc, JSValue *argv) {
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "s3StoreGet(key, fallback)");
    }
    char key[16];
    {
        JSCStringBuf buf;
        ...
        snprintf(key, sizeof(key), "%.*s", static_cast<int>(len), str);
    }
    int fallback = 0;
    if (JS_ToInt32(ctx, &fallback, argv[1])) {
        return JS_EXCEPTION;
    }
    return JS_NewInt32(ctx, SettingsGet(key, fallback));
}

JSValue js_s3_store_set(JSContext *ctx, JSValue *, int argc, JSValue *argv) {
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "s3StoreSet(key, value)");
    }
    char key[16];
    {
        JSCStringBuf buf;
...
// scan picks it up and the Reader can open your own journal.
JSValue js_s3_append_postcard(JSContext *ctx, JSValue *, int argc,
                              JSValue *argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "s3AppendPostcard(line)");
    }
    if (!StorageMounted()) {
        return JS_NewInt32(ctx, static_cast<int32_t>(StatusCode::Busy));
    }
    char line[96];
    {
        ...
        snprintf(line, sizeof(line), "%.*s", static_cast<int>(len), str);
    }
